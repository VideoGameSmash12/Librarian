...
package me.videogamesm12.librarian.v1_14_4.ornithe.listeners;
...
import me.videogamesm12.librarian.v1_14_4.ornithe.mixin.CreativeInventoryScreenAccessor;
...
public class ActionBarNotifier extends AbstractEventListener
{
	...
	public void onPageLoad(AsyncPageLoadEvent event)
	{
		if (!(Minecraft.getInstance().screen instanceof CreativeInventoryScreen
				&& CreativeInventoryScreenAccessor.getSelectedTab() == CreativeModeTab.HOTBAR.getId()
				&& Librarian.getInstance().getCurrentPageNumber().equals(event.getPage().librarian$getPageNumber())))
		{
			sendActionBar(new TranslatableText("librarian.messages.loaded.action_bar",
