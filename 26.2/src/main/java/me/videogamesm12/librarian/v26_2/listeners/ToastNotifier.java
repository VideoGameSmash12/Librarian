...
public class ToastNotifier extends AbstractEventListener
{
	...
	{
		final LibrarianToast toast = Minecraft.getInstance().gui.toastManager().getToast(LibrarianToast.class, type);

		if (toast == null)
		{
			Minecraft.getInstance().gui.toastManager().addToast(new LibrarianToast(title, description, type));
		}
		else
		{
