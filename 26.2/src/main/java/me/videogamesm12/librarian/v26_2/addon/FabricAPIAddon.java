...
package me.videogamesm12.librarian.v26_2.addon;
...
@AddonMeta(requiredMods = "fabric-api")
@Getter
public class FabricAPIAddon implements IAddon
{
	private final IMechanicFactory mechanic = Librarian.getInstance().getMechanic();

	private KeyMapping.Category navigationCategory;
	private KeyMapping.Category actionsCategory;

	private KeyMapping nextKey = null;
	private KeyMapping previousKey = null;
	private KeyMapping backupKey = null;

	@Override
	public void init()
	{
		navigationCategory = KeyMapping.Category.register(Identifier.fromNamespaceAndPath("librarian", "navigation"));
		actionsCategory = KeyMapping.Category.register(Identifier.fromNamespaceAndPath("librarian", "actions"));

		nextKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("librarian.key.next_page",
				InputConstants.Type.KEYSYM,
				GLFW.GLFW_KEY_RIGHT_BRACKET,
				navigationCategory));
		previousKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("librarian.key.previous_page",
				InputConstants.Type.KEYSYM,
				GLFW.GLFW_KEY_LEFT_BRACKET,
				navigationCategory));
		backupKey = KeyMappingHelper.registerKeyMapping(new KeyMapping("librarian.key.backup_current_page",
				InputConstants.Type.KEYSYM,
				GLFW.GLFW_KEY_B,
				actionsCategory));
		...
		if (commandSystem.isEnabled())
		{
			...
			{
				final LiteralCommandNode<FabricClientCommandSource> mainCommand = dispatcher.register(ClientCommands.literal("librarian")
						.then(ClientCommands.literal("goto")
								.then(ClientCommands.argument("page", LongArgumentType.longArg())
										.executes(context ->
										{
						...
						.then(ClientCommands.literal("previous")
								.executes(context ->
								{
									Librarian.getInstance().previousPage();
									return 0;
						...
						.then(ClientCommands.literal("meta")
								...
								.then(ClientCommands.literal("description")
										.executes(context ->
										{
											IWrappedHotbarStorage page = Librarian.getInstance().getCurrentPage();
											loadAndThen(context.getSource(), page.librarian$getPageNumber(), () ->
								...
								.then(ClientCommands.literal("authors")
										.then(ClientCommands.literal("list")
												.executes(context ->
												{
													IWrappedHotbarStorage page = Librarian.getInstance().getCurrentPage();
				...
				commandSystem.getAliases().stream().map(alias -> ClientCommands.literal(alias)
	...
	private void loadAndThen(FabricClientCommandSource source, BigInteger page, Runnable action)
	{
		...
		if (storage.librarian$getLoadStatus() != LoadStatus.LOADED)
		{
			...
			if (storage.librarian$getLoadStatus() == LoadStatus.NOT_LOADED)
			{
				...
				{
					storage.librarian$loadAsync().whenComplete((_, _) -> action.run());
				}
				else
				{
