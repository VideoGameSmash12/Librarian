...
import me.videogamesm12.librarian.v1_13_2.ornithe.mixin.ConfirmScreenAccessor;
...
public class FixedConfirmScreen extends ConfirmScreen
{
	...
	public FixedConfirmScreen(String title, String message, int id, ConfirmationListener callback)
	{
	...
	public void init()
	{
		...
		messageSplit.clear();
		messageSplit.addAll(this.textRenderer.split(message, this.width - 50));

		int buttonY = MathHelper.clamp(getButtonY(), this.height / 6 + 96, this.height - 24);
		this.addButton(new ButtonWidget(0, this.width / 2 - 155, buttonY, 150, 20, this.confirmText)
		{
			public void click(double mouseX, double mouseY)
			{
				shortcut.getParent().confirmResult(true, this.id);
			}
		});
		this.addButton(new ButtonWidget(1, this.width / 2 - 155 + 160, buttonY, 150, 20, this.abortText)
		{
			public void click(double mouseX, double mouseY)
			{
				shortcut.getParent().confirmResult(false, this.id);
			}
		});
	}
