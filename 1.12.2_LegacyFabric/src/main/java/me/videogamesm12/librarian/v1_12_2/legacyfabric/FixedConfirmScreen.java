...
import me.videogamesm12.librarian.v1_12_2.legacyfabric.mixin.ConfirmScreenAccessor;
...
public class FixedConfirmScreen extends ConfirmScreen
{
	...
	public FixedConfirmScreen(IdentifiableBooleanConsumer identifiableBooleanConsumer, String string, String string2, int i)
	{
		super(identifiableBooleanConsumer, string, string2, i);
	...
	public void init()
	{
		...
		shortcut.getLines().addAll(this.textRenderer.wrapLines(shortcut.getSubtitle(), this.width - 50));

		int buttonY = MathHelper.clamp(this.getMessageY() + this.getMessagesHeight() + 20, this.height / 6 + 96, this.height - 24);

		this.buttons.add(new OptionButtonWidget(0, this.width / 2 - 155, buttonY, this.yesText));
		this.buttons.add(new OptionButtonWidget(1, this.width / 2 - 155 + 160, buttonY, this.noText));
	}
	...
	public void render(int mouseX, int mouseY, float tickDelta)
	{
		...
		buttons.forEach(button -> button.method_891(this.client, mouseX, mouseY, tickDelta));
		labels.forEach(labelWidget -> labelWidget.render(this.client, mouseX, mouseY));
	}
